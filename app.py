...
def master(url, model) :
    ...
    parts = url.split('www.', 1)
    if len(parts) > 1:
        domain, rest = parts[1].split('/', 1)
    else:
        domain, rest = parts[0], ''

    # Split the rest into directory, file, and parameters
    path_parts = rest.split('?', 1)
    directory, file = path_parts[0].rsplit('/', 1) if '/' in path_parts[0] else ('', path_parts[0])
    if len(path_parts) > 1:
    ...
    with st.spinner("Counting characters, vowels....") :
        count_characters_and_vowels(url, domain, directory, file, parameters, data)
...
def count_characters_and_vowels(url, domain, directory, file, parameters, data):
...
def get_ip_resolved(domain, data):
    try:
        ip_addresses = sockets.gethostbyname_ex(domain)
